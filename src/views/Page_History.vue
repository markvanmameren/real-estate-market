<template>
  <div class="page history">
    ...
    <div class="houses-container">
      <p
        class="searchCount"
        v-if="this.searchTerm && this.historyHousesCount > 0"
      >
        {{ this.historyHousesCount + " results found" }}
      </p>
      <HouseColumn
        v-for="house in this.historyHouses"
        :key="house.id"
        :house="house"
      />
      <NoResults v-if="this.historyHousesCount == 0" :msg="this.noResultsMsg" />
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    historyIds() {
      return this.$store.getters.getHistoryIds;
    },
    historyHouses() {
      return this.$store.getters.getHistoryHouses;
    },
    historyHousesCount() {
      return this.$store.getters.getHistoryCount;
    },
    searchTerm() {
    ...
    noResultsMsg() {
      return "You have not yet viewed any houses.";
    },
  },
  methods: {
